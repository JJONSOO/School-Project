...
def keygen(keylen):
    ...
    half_len = keylen//2
    p = random.randint(2 ** (half_len), 2 ** (half_len + 1) )
    q = random.randint(2 ** (half_len), 2 ** (half_len + 1) )
    while miller_rabin(p, 20) != Prime :
        p = random.randint(2 ** (half_len), 2 ** (half_len+1))
    while p == q or miller_rabin(q, 20) != Prime:
        q = random.randint(2 ** (half_len), 2 ** (half_len+1))

    n = q * p
    exn = (p-1) * (q-1)
    e = random.randint(2,exn+1)
    while e == exn or gcd(exn, e) != 1:
        e = random.randint(1, exn+1)
    d = m_inv(e, exn)
